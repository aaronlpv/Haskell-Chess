module Parsing (parseDotChess, Disamb(..), PawnExtra(..), ParsedMove(..), CheckStatus(..), ParsedNode(..), ParsedNext(..), ParsedState(..)) where
import Types
...
instance NFData ParsedMove where
  ...
  rnf (ParsedMove dis pc cap sq paw ch) = rnf dis `seq` pc `seq` cap `seq` rnf sq `seq` rnf paw `seq` ch `seq` ()
...
pPiece = choice [char 'K' >> return King,
                 char 'Q' >> return Queen,
                 char 'R' >> return Rook,
