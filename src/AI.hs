module AI(AITree(..), doAI, findByMove, scoreTree, bestForPlayer, annotateStep, treeToString, applyAnnotated, aiFromParsed) where
import Types
import Board
import Position
import Chess
...
applyAnnotated state (m:ms) =
  applyAnnotated (stateDoMove state move) ms
...
findMoveFromParsed state _ (ParsedCastling side) =
  castleMove (turn state) side
findMoveFromParsed state moves (ParsedMove dis pc _ dest _ _) =
  fromJust (find (matchDis dis) pcTo)
  where pcTo = filterPieceTo (board state) pc dest moves
...
aiFromParsed state =
  map (aiFromParsedRec state legals)
  where legals = legalMoves state
...
aiFromParsedRec state legals (ParsedNode pmove pnext) =
  ...
  where move = findMoveFromParsed state legals pmove
